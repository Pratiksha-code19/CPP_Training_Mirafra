...
public:
    ...
    void vehicleEnter(string type, int entryTime) {
        ...
        if (parkingLot.size() < totalSpaces) {
            ...
            // If parking lot is full, add to the waiting queue with priority for emergency vehicles
            if (type == "emergency") {
                // Insert emergency vehicle in front of the queue
                queue<Vehicle> tempQueue;
                tempQueue.push(newVehicle);

                // Re-enqueue normal vehicles
                ...
                cout << "Emergency vehicle entered at time " << entryTime << " and is waiting." << endl;
            } else {
                waitingQueue.push(newVehicle);
                cout << "Normal vehicle entered at time " << entryTime << " and is waiting." << endl;
            }
        }
    }
