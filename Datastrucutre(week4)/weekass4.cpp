...
private:
    ...
    queue<Vehicle> vipQueue;          // VIP vehicles queue
...
public:
    ...
    void parkVehicle(Vehicle vehicle) {
        if (vehicle.type == EMERGENCY || vehicle.type == VIP) {
            if (vehicle.type == EMERGENCY) {
                ...
                vipQueue.push(vehicle);
                cout << "VIP vehicle " << vehicle.plateNumber << " added to priority queue." << endl;
            }
        } else {
    ...
    // Function to park the next available vehicle from priority queue or waiting area
    void parkNextVehicle() {
        ...
        if (!emergencyQueue.empty()) {
            ...
            emergencyQueue.pop();
        } else if (!vipQueue.empty()) {
            vehicle = vipQueue.front();
            vipQueue.pop();
    ...
    void displayStatus() {
        ...
        cout << "Vehicles in VIP queue: " << vipQueue.size() << endl;
...
int main() {
    ...
    Vehicle car3("VIP555", VIP);
    Vehicle car4("XYZ456", NORMAL);
    
    parkingSystem.parkVehicle(car1);
    parkingSystem.parkVehicle(car2);
    parkingSystem.parkVehicle(car3);
    parkingSystem.parkVehicle(car4);

    // Park vehicles
    parkingSystem.parkNextVehicle(); // Emergency or VIP vehicles will be parked first
